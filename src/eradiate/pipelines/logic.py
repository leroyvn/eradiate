...
def spectral_response_weights(
    ...
    Notes
    -----
    SRF weighting is a linear map from per-bin values to a single band value.
    The weights are obtained by pushing the identity over the spectral grid of
    `spectral_data` through the very resampling and integration
    :func:`apply_spectral_response` applies to the data, so that they are by
    construction the coefficients of that map.

    Materialising them is what makes variance propagation possible. Resampling
    replicates the value of a bin over every node of the merged grid that is
    nearest to it, and those replicas are perfectly correlated; squaring the
    node weights would therefore understate the variance. Squaring these
    per-bin coefficients does not.
    """
...
def apply_spectral_response(
    ...
    try:
        # A variance is named after the variable it qualifies, not after the
        # data array it is computed from: '<var>_var' → '<var>_srf_var', so
        # that it pairs with '<var>_srf' the way '<var>_var' pairs with '<var>'
        name = (
